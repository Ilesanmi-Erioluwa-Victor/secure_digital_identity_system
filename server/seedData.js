  ...
  try {
    ...
    const settings = await Settings.create({
      ...
      accessLevelDescriptions: [
        ...
        { level: 3, description: 'Elevated - Staff access with all privileges' },
    ...
    const makeVerifyToken = () => {
      ...
      const { rawToken, hmacSignature, verifyUrl } = generateVerifyToken(fakeId);
    ...
    const identitiesData = [
      {
        ...
        lifecycleHistory: [
          { action: 'Issued', performedBy: adminUser._id, reason: 'Initial issuance', timestamp: now },
        ],
      },
      {
      ...
      },
      {
        ...
        lifecycleHistory: [
          { action: 'Issued', performedBy: adminUser._id, reason: 'Initial issuance', timestamp: now },
          { action: 'Suspended', performedBy: adminUser._id, reason: 'Lost identity card - reported', timestamp: now },
        ],
      },
      {
        ...
        lifecycleHistory: [
          { action: 'Issued', performedBy: adminUser._id, reason: 'Initial issuance', timestamp: now },
          { action: 'Revoked', performedBy: adminUser._id, reason: 'Upgraded to new identity card', timestamp: now },
        ],
      },
      {
        ...
        issuedBy: adminUser._id,
        lifecycleHistory: [
          { action: 'Issued', performedBy: adminUser._id, reason: 'Initial issuance', timestamp: lastYear },
          { action: 'Expired', performedBy: null, reason: 'Auto-expired', timestamp: lastYear },
        ],
      },
    ];
